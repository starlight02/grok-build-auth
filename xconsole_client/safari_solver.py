...
Chrome-like quiet mode (default ON):
  CF rejects in-page synthetic clicks in Safari, so a real HID click is still
  required — but the window does **not** need to stay frontmost.
  Idle: park off-screen and/or minimize (like Drission ``TURNSTILE_MINIMIZED``).
  Click: flash-activate Safari ~0.3s → HID click → restore previous app.
...
Env:
  ...
  TURNSTILE_SAFARI_BACKGROUND=1  (default) flash-focus click + quiet window
  TURNSTILE_OFFSCREEN=1          (default) park Safari off-screen when idle
  TURNSTILE_MINIMIZED=1          (default) minimize Safari when idle
  TURNSTILE_SAFARI_FLASH_HOLD=0.35  seconds Safari stays frontmost after click
  TURNSTILE_SAFARI_BOUNDS=x1,y1,x2,y2  on-screen bounds used only for HID click
  TURNSTILE_REQUIRE_FRONTMOST=0  (legacy; background mode ignores permanent lock)
...
class SafariTurnstileSolver:
    """Solve Turnstile with system Safari via Apple Events JavaScript.

    CF rejects in-page synthetic clicks. Strategy (Chrome-like quiet UX):

    1. Force-render Turnstile into a fixed content-viewport host.
    2. Keep Safari quiet while idle (off-screen +/or minimized).
    3. Flash-activate → real HID click on mapped screen coords → restore focus.
    4. Optional ``TURNSTILE_INTERACTIVE=1`` human-click fallback.
    """
    ...
    def __init__(
        ...
        **_ignored: Any,
        ...
        # Chrome-parity quiet defaults: background flash + offscreen + minimize.
        self._background = _env_truthy("TURNSTILE_SAFARI_BACKGROUND", True)
        self._offscreen = _env_truthy("TURNSTILE_OFFSCREEN", True)
        self._minimized = _env_truthy("TURNSTILE_MINIMIZED", True)
        try:
            self._flash_hold = float(
                (os.environ.get("TURNSTILE_SAFARI_FLASH_HOLD") or "0.35").strip()
            )
        except ValueError:
            self._flash_hold = 0.35
        self._flash_hold = max(0.15, min(self._flash_hold, 3.0))
        # On-screen bounds used only during the flash click (content must be hit-testable).
        self._click_bounds = self._parse_bounds(
            (os.environ.get("TURNSTILE_SAFARI_BOUNDS") or "").strip()
            or "900,500,1400,900"
        )
        self._offscreen_bounds = (-2200, 60, -1000, 760)
        if self._proxy:
            self._log(
                "HTTPS_PROXY set but Safari backend uses system proxy only; ignored"
            )
    ...
    def _log(self, msg: str) -> None:
        if self._debug:
            ...
            low = msg.lower()
            if (
                low.startswith("pause")
                or "skip click" in low
                or low.startswith("hid ")
            ):
                print(f"[SafariTurnstile] {msg}", flush=True)

    def _pause_file(self) -> str:
        return (
            os.environ.get("TURNSTILE_PAUSE_FILE") or "/tmp/grok-turnstile.pause"
        ).strip()
    ...
    def _require_frontmost(self) -> bool:
        """Legacy permanent-frontmost lock. Off by default when background mode on."""
        default = "0" if self._background else "1"
        raw = (os.environ.get("TURNSTILE_REQUIRE_FRONTMOST") or default).strip().lower()
    ...
    def _set_frontmost_app(self, name: str) -> None:
        name = (name or "").strip()
        if not name:
            return
...
tell application "System Events"
  try
    set frontmost of first process whose name is {_as_quote(name)} to true
  end try
end tell
'''
        try:
            _run_osascript(src, timeout=5.0)
        except Exception:
            pass
...
'''
    ...
    def _quiet_window(self) -> None:
        """Park Safari out of the way (Chrome-like minimized/offscreen)."""
        if not (self._background or self._offscreen or self._minimized):
            return
        if self._offscreen:
            self._set_window_bounds(self._offscreen_bounds)
        if self._minimized:
            self._set_minimized(True)

    def _prepare_window_for_click(self) -> None:
        """Unminimize + put Safari on-screen so HID can hit the widget."""
        self._set_minimized(False)
        self._set_window_bounds(self._click_bounds)

    def _flash_safari_for_click(self, *, deadline: float) -> Optional[str]:
        """Activate Safari briefly for HID; return previous frontmost app name."""
        prev = self._frontmost_app_name()
        ...
        for _ in range(25):
            ...
            if self._safari_is_frontmost():
                return prev
            time.sleep(0.03)
        if not self._safari_is_frontmost():
            self._log("flash activate: Safari not frontmost yet; clicking anyway")
        return prev

    def _restore_after_flash(self, prev: Optional[str]) -> None:
        time.sleep(self._flash_hold)
        if prev and prev.lower() != "safari":
            self._set_frontmost_app(prev)
        self._quiet_window()

    def _ensure_safari_frontmost(self, *, deadline: float) -> bool:
        """Legacy helper: permanent frontmost only when background mode is off."""
        if self._background and not self._require_frontmost():
            return True
        try:
            _run_osascript('tell application "Safari" to activate', timeout=5.0)
        except Exception as exc:
        ...
        if not ok:
            self._log(
                "skip click: Safari is not frontmost "
                "(focus another app / use pause file)"
            )
        return ok
...
'''
    ...
    def _open(self, url: str) -> None:
        """Load URL without permanently stealing focus; quiet window afterwards."""
        prev = self._frontmost_app_name() if self._background else ""
        # Open without activate when possible; first window may still need one.
        src = f'''
tell application "Safari"
  if (count of windows) is 0 then
...
'''
        try:
            ...
            # Cold Safari sometimes needs activate to create a window.
            _run_osascript(
                f'''
tell application "Safari"
  activate
...
'''
    ...
    def _widget_viewport_box(self) -> Optional[dict[str, float]]:
        """Viewport box of Turnstile host / iframe (CSS pixels)."""
        js = r"""
(() => {
  const pick = (el) => {
    if (!el) return null;
  ...
  if (box) return JSON.stringify(box);
  const nodes = [
...
'''
    ...
    def _try_os_click_widget(self) -> None:
        """Map widget viewport box → screen coords and click the checkbox."""
        bounds = self._window_bounds()
        box = self._widget_viewport_box()
        if not bounds:
            self._log("os-click skip: no window bounds")
            return
        x1, y1, x2, y2 = bounds
        chrome_top = self._content_chrome_top(bounds)
        if not box:
            # Fallback: force-render host is fixed at (16,16) 300x80.
            box = {"x": 16.0, "y": 16.0, "w": 300.0, "h": 80.0}
            self._log("os-click using fixed host fallback box")
        # Checkbox is left side of the Turnstile card, vertically centered.
        vx = box["x"] + min(28.0, max(18.0, box["w"] * 0.10))
        vy = box["y"] + max(14.0, box["h"] * 0.50)
        sx = int(round(x1 + vx))
        sy = int(round(y1 + chrome_top + vy))
        sx = max(x1 + 5, min(x2 - 5, sx))
        sy = max(y1 + chrome_top + 5, min(y2 - 5, sy))
        self._log(
            f"os-click screen=({sx},{sy}) widget=({box['x']:.0f},{box['y']:.0f},"
            f"{box['w']:.0f}x{box['h']:.0f}) chrome_top={chrome_top}"
        )
        res = self._os_click_screen(sx, sy)
        self._log(f"os-click result: {res}")
        if res.startswith("click-failed"):
            self._log(
                "Mouse click needs Accessibility permission for this app "
                "(System Settings → Privacy & Security → Accessibility)"
            )

    def _click_widget_until_token(self, deadline: float, *, label: str) -> str:
        """Flash-focus HID click (Chrome-like quiet) with jitter; poll token."""
        offsets = (
            (0, 0),
            (0, 2),
            (-2, 0),
        ...
        for i, (dx, dy) in enumerate(offsets):
            ...
            try:
                if self._background and not self._require_frontmost():
                    prev = self._flash_safari_for_click(deadline=deadline)
                elif not self._ensure_safari_frontmost(deadline=deadline):
                    time.sleep(0.5)
                    continue
                else:
                    self._prepare_window_for_click()
                ...
                self._log(
                    ...
                    f"bg={int(self._background)} front={self._frontmost_app_name()!r}"
                )
                self._os_click_screen(sx, sy)
            finally:
                if self._background and not self._require_frontmost():
                    self._restore_after_flash(prev)
    ...
    def solve_turnstile(
        ...
        *,
        ...
        self._log(
            ...
            f"interactive={self._interactive} background={self._background} "
            f"offscreen={self._offscreen} minimized={self._minimized} "
            f"flash_hold={self._flash_hold:.2f}s"
        )
        if self._background:
            # Honest: CF requires a real HID click with Safari frontmost briefly.
            # This WILL steal mouse/focus for ~flash_hold. Quiet batch → drission.
            print(
                "  [SafariTurnstile] WARN: Safari mint steals OS mouse/focus for "
                f"~{self._flash_hold:.2f}s per token. For unattended/quiet mint use "
                "TURNSTILE_SOLVER=drission (CDP in-browser click, no OS mouse).",
                flush=True,
            )
        self._wait_if_paused(deadline)
        if time.time() >= deadline - 3:
            raise TimeoutError(
                f"Safari Turnstile paused until timeout "
                f"(remove {self._pause_file()} to resume)"
            )
    ...
    def _solve_once(self, url: str, key: str, deadline: float) -> str:
        ...
        # Email path (helps page settle on signup form).
        try:
        ...
        # Force-render needs a live document; unminimize but stay off-focus/offscreen.
        if self._background:
            self._set_minimized(False)
            if self._offscreen:
                self._set_window_bounds(self._offscreen_bounds)
        ...
        self._log(
            f"HID click budget {click_budget:.0f}s "
            f"mode={'flash' if self._background and not self._require_frontmost() else 'frontmost'}"
        )
